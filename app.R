...
# Read the data
data <- read.csv("D:/BaiduSyncdisk/UM/WQD7001/group/modified_finaldataset.csv")
  ...
  observeEvent(input$go, {
    if (input$visualization == "map") {
      ...
      output$prediction_plot <- renderPlot({
        ...
        required_columns <- c("DAQI", "AQI_PM2.5", "AQI_PM10", "AQI_O3", "AQI_NO2", "AQI_SO2")
        ...
        train_data <- country_data %>% filter(Date <= as.Date("2024-10-01"))
        test_data <- country_data %>% filter(Date > as.Date("2024-10-01"))
        ...
        #sarima_model <- auto.arima(y_train, xreg = train_exog)
        ...
        ggplot() +
          ...
          labs(
            title = paste("DAQI Prediction for", input$prediction_country, "using SARIMAX with External Variables"),
            x = "Date",
            y = "DAQI"
          ) +
          theme_minimal() +
